...
FlarmMessage::FlarmMessage(const char *data, size_t len)
    : _len(len), _valid(true), _alarmLevel(0), _isTraffic(false), _isHeartbeat(false), _hasAdvisory(false), _hasGps(false), _id(0)
{
    assert(this);
...
/// @brief Extracts whether this message has a traffic advisory.
/// @note Only valid for heartbeat messages.
/// @return true if has traffic advisory (alarm type == 4)
...
/// @brief Extracts whether the GPS is valid.
/// @return 0: no GPS reception, 1: fix on ground,2: fix when airborne.
int FlarmMessage::extractGps()
{
    assert(this);
    assert(isHeartbeat());
